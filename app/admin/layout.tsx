      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <Link href="/admin" style={{ textDecoration: 'none', color: 'var(--text-main)' }}>
            <h1 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700 }}>관리자 대시보드</h1>
          </Link>
          <nav style={{ display: 'flex', gap: '1.5rem' }}>
            {isSuperAdmin && (
              <Link href="/admin/users" style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', textDecoration: 'none' }}>회원 관리</Link>
            )}
            <Link href="/admin/reservations" style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', textDecoration: 'none' }}>예약 관리</Link>
          </nav>
        </div>
